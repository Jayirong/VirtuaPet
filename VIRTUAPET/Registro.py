...
lbltipo=Label(marco, text="Perfil De Usuario").grid(column=3,row=1,padx=5,pady=5)
txttipo=ttk.Combobox(marco,values=["Administrador","Veterinario","Recepcion"], textvariable=Tipo)
txttipo.grid(column=4,row=0)
txttipo.current(0)
...
tvpersonas.grid(column=1,row=5,columnspan=4,padx=5)
tvpersonas["columns"]=("Id","User","Clave","Tipo")
tvpersonas.column("#0",width=0,stretch=NO)
tvpersonas.column("Id",width=10,anchor=CENTER)
tvpersonas.column("User",width=30,anchor=CENTER)
tvpersonas.column("Clave",width=50,anchor=CENTER)
tvpersonas.column("Tipo",width=80,anchor=CENTER)
